"""Deep MLP Stock Predictor - TensorFlow/Keras Implementation"""
import sys
...
OUTPUT_PLOT_PATH = 'predictions_deepmlp.png'
OUTPUT_METRICS_PATH = 'walk_forward_metrics_deepmlp.csv'
...
def walk_forward_validation(df, features, n_splits, min_train_size):
    ...
    for split_num in range(n_splits):
        ...
        print(f"Training Deep MLP model...")
...
def main():
    print("="*80 + "\nDEEP MLP (KERAS) STOCK PREDICTOR\n" + "="*80)
    ...
    final_model.fit(X_all_scaled, y_all, epochs=EPOCHS, batch_size=BATCH_SIZE, verbose=0)
    ...
    if len(df_future_for_pred) > 0:
        ...
        y_future_proba = final_model.predict(X_future_scaled, verbose=0).flatten()
    ...
    ax.set_title(f'{STOCK_NAME} - Deep MLP (Keras) Buy Signals', fontsize=16, fontweight='bold', pad=20)
