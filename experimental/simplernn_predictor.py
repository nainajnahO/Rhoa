"""SimpleRNN Stock Predictor - TensorFlow/Keras Implementation"""
import sys
...
OUTPUT_PLOT_PATH = 'predictions_simplernn.png'
OUTPUT_METRICS_PATH = 'walk_forward_metrics_simplernn.csv'

def create_sequences(X, y, timesteps):
    """Create sliding window sequences for SimpleRNN"""
...
def create_model(n_features):
    model = Sequential([
        SimpleRNN(64, return_sequences=True, input_shape=(TIMESTEPS, n_features)),
        Dropout(0.2),
        SimpleRNN(32),
        Dropout(0.2),
...
def walk_forward_validation(df, features, n_splits, min_train_size):
    ...
    for split_num in range(n_splits):
        ...
        print(f"Training SimpleRNN model...")
...
def main():
    ...
    if len(df_future_for_pred) >= TIMESTEPS:
        ...
        # Need to append last few rows from training for sequence creation
    ...
    ax.set_title(f'{STOCK_NAME} - SimpleRNN Buy Signals', fontsize=16, fontweight='bold', pad=20)
