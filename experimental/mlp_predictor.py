"""
MLP (Multi-Layer Perceptron) Stock Predictor - Fixed Implementation

This script trains an MLP neural network model using proper time series validation to prevent data leakage.
...
Usage:
    python experimental/mlp_predictor.py
"""
...
# MLP Hyperparameters (optimized for accuracy)
MLP_PARAMS = {
...
# Visualization
OUTPUT_PLOT_PATH = 'predictions_mlp.png'
OUTPUT_METRICS_PATH = 'walk_forward_metrics_mlp.csv'
...
# ============================================================================
# WALK-FORWARD VALIDATION WITH FEATURE SCALING
# ============================================================================

def walk_forward_validation(df, features, n_splits, min_train_size):
    """
    Perform walk-forward validation with expanding window and feature scaling.
    """
    total_rows = len(df)
    results = []
...
def main():
    print("="*80)
    print("MLP STOCK PREDICTOR - FIXED IMPLEMENTATION")
    print("="*80)
    ...
    # Feature Importance (MLP doesn't have feature_importances_, skip)
    print("### Step 7: Feature Importance ###")
    print("⚠️  MLP doesn't provide feature importance (neural network black box)")
    print()
    ...
    final_model = MLPClassifier(**MLP_PARAMS)
    ...
    if START_DATE is not None or END_DATE is not None:
        ...
        if START_DATE is not None:
            date_mask &= (df_historical['Date'] >= pd.to_datetime(START_DATE))
        if END_DATE is not None:
            date_mask &= (df_historical['Date'] <= pd.to_datetime(END_DATE))
    ...
    title = f'{STOCK_NAME} - MLP Buy Signals (Walk-Forward + Future)'
