"""
SVM (Support Vector Machine) Stock Predictor - Fixed Implementation

This script trains an SVM model using proper time series validation to prevent data leakage.
...
Usage:
    python experimental/svm_predictor.py
"""
...
OUTPUT_PLOT_PATH = 'predictions_svm.png'
OUTPUT_METRICS_PATH = 'walk_forward_metrics_svm.csv'
START_DATE = None
END_DATE = None
...
def main():
    print("="*80)
    print("SVM STOCK PREDICTOR - FIXED IMPLEMENTATION")
    print("="*80)
    ...
    df_clean = df_clean.dropna(subset=['target']).dropna(subset=available_features)
    ...
    print("⚠️  SVM doesn't provide direct feature importance")
    print()
    ...
    title = f'{STOCK_NAME} - SVM Buy Signals (Walk-Forward + Future)'
