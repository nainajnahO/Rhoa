"""Attention-LSTM Stock Predictor - TensorFlow/Keras Implementation"""
import sys
...
from tensorflow.keras.layers import LSTM, Dense, Dropout, Input, Permute, Multiply, Lambda, Flatten
...
OUTPUT_PLOT_PATH = 'predictions_attentionlstm.png'
OUTPUT_METRICS_PATH = 'walk_forward_metrics_attentionlstm.csv'

def create_sequences(X, y, timesteps):
    """Create sliding window sequences for Attention-LSTM"""
    ...
    for i in range(n_samples):
        ...
        y_seq[i] = y[i+timesteps-1]
    return X_seq, y_seq

def create_model(n_features):
    """Create Attention-LSTM model with attention mechanism"""
    ...
    sent_representation = Lambda(lambda x: tf.keras.backend.sum(x, axis=1))(sent_representation)
    
    # Output layers
    dense = Dense(16, activation='relu')(sent_representation)
...
def walk_forward_validation(df, features, n_splits, min_train_size):
    ...
    for split_num in range(n_splits):
        ...
        print(f"Creating sequences (timesteps={TIMESTEPS})...")
        ...
        df_test_seq = df_test.iloc[TIMESTEPS-1:].reset_index(drop=True)
        
        print(f"Training Attention-LSTM model...")
        ...
        precision, recall, f1, _ = precision_recall_fscore_support(y_test_seq, y_pred, average='binary', zero_division=0)
        ...
        results.append({
            ...
            'test_start': df_test_seq['Date'].min(),
            'test_end': df_test_seq['Date'].max(),
            'n_train': len(y_train_seq),
...
def main():
    print("="*80 + "\nATTENTION-LSTM STOCK PREDICTOR\n" + "="*80)
    ...
    X_all_seq, y_all_seq = create_sequences(X_all_scaled, y_all, TIMESTEPS)
    
    final_model = create_model(X_all_scaled.shape[1])
    final_model.fit(X_all_seq, y_all_seq, epochs=EPOCHS, batch_size=BATCH_SIZE, verbose=0)
    
    df_future_for_pred = df_future_clean.dropna()
    if len(df_future_for_pred) >= TIMESTEPS:
        X_future = df_future_for_pred[available_features].values
        X_future_scaled = final_scaler.transform(X_future)
        
        X_combined = np.vstack([X_all_scaled[-(TIMESTEPS-1):], X_future_scaled])
        y_dummy = np.zeros(len(X_combined))
        X_future_seq, _ = create_sequences(X_combined, y_dummy, TIMESTEPS)
        
        y_future_proba = final_model.predict(X_future_seq, verbose=0).flatten()
    ...
    if df_future_for_pred is not None and len(df_future_for_pred) >= TIMESTEPS:
    ...
    ax.set_title(f'{STOCK_NAME} - Attention-LSTM Buy Signals', fontsize=16, fontweight='bold', pad=20)
